...
BlockMatrix::BlockMatrix (const BlockMatrix &other):
    matrices_             (new Matrix* [other.width_ * other.height_]),
...
BlockMatrix::BlockMatrix (size_t height, size_t width, size_t internal_matrix_size) :
    BlockMatrix (support_array_for_constructor_ = new matrix_t[height * width] /*(matrix_t*)std::calloc(height * width, sizeof(matrix_t))*/, height, width, internal_matrix_size) {
    delete[] support_array_for_constructor_;
    support_array_for_constructor_ = nullptr;
}

BlockMatrix::BlockMatrix (matrix_t *values, size_t height, size_t width, size_t internal_matrix_size) :
    internal_matrix_size_ (internal_matrix_size) {
    ...
    for (size_t i = 0; i < (height_ - addition_matrix_bot) * width_; ++i) {
        if (addition_matrix_right && i % width_ == width_ - 1) { // for last column
        ...
        for (size_t block_col = 0; block_col < width_; ++block_col) {
            ...
            for (size_t matrix_row = 0; matrix_row < this_matrix->getHeight(); ++matrix_row) {
                for (size_t matrix_col = 0; matrix_col < this_matrix->getWidth(); ++matrix_col) {
                    (*this_matrix)[matrix_row][matrix_col] = (block_row * internal_matrix_size_ + matrix_row) * dataWidth
                                                           + (block_col * internal_matrix_size_ + matrix_col);
                }
            }
        }
    }
...
void threadMul (Matrix** result_matrix, size_t start_row, size_t finish_row, const BlockMatrix& this_matrix, const BlockMatrix& other_matrix) {
    ...
    for (size_t row = 0; row < finish_row - start_row; ++row) {
        Matrix**       result_row = result_matrix + row;
        ...
        for (size_t column = 0; column < this_matrix.width_; ++column) {
            ...
            std::sprintf(buff, "%f %f\n", this_row_column[0][0], *other_row[0][0][0]);
            std::cout << buff << std::flush;
...
BlockMatrix BlockMatrix::multiThreadMul (const BlockMatrix &other, size_t num_of_threads) const {
    ...
    for (size_t i = 0; i < num_of_threads; ++i)
        threads[i] = std::thread( threadMul,
                                  result.matrices_ + i * height_ / num_of_threads * width_,
...
void threadMulSplitted (Matrix** result_matrix, size_t start_row, size_t finish_row, const BlockMatrix& this_matrix, const BlockMatrix& other_matrix) {
    char buff[128];
    std::sprintf(buff, "%ld %ld: I do it!\n", start_row, finish_row);
    std::cout << buff << std::flush;
...
BlockMatrix BlockMatrix::multiThreadMulSplitted (const BlockMatrix &other, size_t num_of_threads) const {
    if (width_ != other.height_ || internal_matrix_size_ != other.internal_matrix_size_) {
        throw(std::runtime_error(__PRETTY_FUNCTION__ + std::string(": error, matrices can not be multiplied\n")));
        return BlockMatrix();
    }
